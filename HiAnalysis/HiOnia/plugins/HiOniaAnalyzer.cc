...
// $Id: HiOniaAnalyzer.cc,v 1.23.2.20 2013/05/28 12:37:38 tdahms Exp $
//
//
...
private:
  ...
  static const int Max_cal_size = 10000;
  ...
  int Reco_cal_size;       // Number of CaloCand 
  ...
  int Reco_QQ_sign[Max_QQ_size];   /* Mu Mu combinations sign:
			     ...
			     2 = -/- 
			  */
  bool Reco_QQ_exclcalo[Max_QQ_size];   //exclusive calo cut
  ...
  double CaloTower_hadE[Max_cal_size];
  double CaloTower_emE[Max_cal_size];
  double CaloTower_e[Max_cal_size];
  double CaloTower_et[Max_cal_size];
  double CaloTower_eta[Max_cal_size];
  double CaloTower_phi[Max_cal_size];
  double CaloTower_dr[Max_cal_size];
  /*
...
//  CaloTowerCollection::const_iterator calo;
  ...
  int _oniaPDG;
  unsigned int _lowlimitRun, _upplimitRun;
...
HiOniaAnalyzer::HiOniaAnalyzer(const edm::ParameterSet& iConfig):
  ...
  _theCaloTowLabel(iConfig.getParameter<edm::InputTag>("CaloTowerLabel")),
  ...
  _lowlimitRun(iConfig.getParameter<uint>("lowlimitRun")),
  _upplimitRun(iConfig.getParameter<uint>("upplimitRun")),
  _iConfig(iConfig)
{
	cout << " Version A " << endl;
...
HiOniaAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if( runNb < _lowlimitRun || runNb > _upplimitRun )
    return;
  ...
  iEvent.getByLabel(_theCaloTowLabel,caloTowers);
  if (_isMC) {
  ...
  if (collTracks.isValid()) {
	...
	it!=collTracks->end(); ++it) {
      const reco::Track* track = &(*it);	
       ...
       /*cout << endl << endl << endl;
       cout << "========================================================================" << endl;
       cout << "Track qualityByName = " << track->qualityByName("highPurity") << endl;
       cout << "Track quality Name  = ";
       //for (reco::TrackBase::TrackQuality i = reco::TrackBase::TrackQuality::undefQuality; i!=reco::TrackBase::TrackQuality::qualitySize; i=i+1) cout << track->quality(i) << " "; cout << endl;
       //for (int i = 0; i<9; i++) cout << track->quality(reco::TrackBase::TrackQuality::undefQuality+i) << " "; cout << endl;
       cout << "Track pT            = " << track->pt() << endl;
       cout << "Track abs(eta)      = " << fabs(track->eta()) << endl;
       cout << "Track Sigma-pT/pT   = " << track->ptError()/track->pt() << endl;
       cout << "Track dr/drsigma    = " << fabs(dz/dzsigma) << " " << fabs(dxy/dxysigma) << endl;
       cout << "========================================================================" << endl;
       cout << endl << endl << endl;*/ //abs(dz_pl)<3.0 && abs(dxy_pl)<0.3 :)!changed

       if( track->qualityByName("highPurity") &&
	   track->pt()>0.4 && fabs(track->eta())<2.4 &&
	   track->ptError()/track->pt()<0.1 && fabs(dz)<3.0 && fabs(dxy)<0.3)//changedfabs(dz/dzsigma)<3.0 && fabs(dxy/dxysigma)<0.3
					        {
   ...
   Reco_QQ_exclcalo[Reco_QQ_size] = 0;
   bool     passExclRecoCuts =false; 
   double leadingEmEnergy_EB=0.0;
   double leadingEmEnergy_EE=0.0;
   double leadingEmEnergy_HB=0.0;
   double leadingEmEnergy_HE=0.0;
   double leadingEmEnergy_HF_Plus=0.0;
   double leadingEmEnergy_HF_Minus=0.0;
   double leadingHadEnergy_EB=0.0;
   double leadingHadEnergy_EE=0.0;
   double leadingHadEnergy_HB=0.0;
   double leadingHadEnergy_HE=0.0;
   double leadingHadEnergy_HF_Plus=0.0;
   double leadingHadEnergy_HF_Minus=0.0;
   int nextracalotowersE4=0;
   double deta,dphi,dr;
   double dr_min;


  for (std::vector<CaloTower>::const_iterator it=caloTowers->begin();
        it!=caloTowers->end(); ++it )
//  for (calo = towers->begin(); calo != towers->end(); ++calo )
  {
          dr_min = 99;
          const CaloTower* calo = &(*it);
          CaloTower_emE[Reco_cal_size]=calo->emEnergy();
          CaloTower_hadE[Reco_cal_size]=calo->hadEnergy();
          CaloTower_e[Reco_cal_size]=calo->energy();
          CaloTower_et[Reco_cal_size]=calo->et();
          CaloTower_phi[Reco_cal_size]=calo->phi();
          CaloTower_eta[Reco_cal_size]=calo->eta();
//          std::cout<<"emEnergy "<<CaloTower_emE[Reco_cal_size]<<"hadEnergy "<< CaloTower_hadE[Reco_cal_size]<<CaloTower_e[Reco_cal_size]<<std::endl;
          float calodr1 = sqrt(((CaloTower_eta[Reco_cal_size]-iTrack_mupl->eta())*(CaloTower_eta[Reco_cal_size]-iTrack_mupl->eta())) +
                               ((CaloTower_phi[Reco_cal_size]-iTrack_mupl->phi())*(CaloTower_phi[Reco_cal_size]-iTrack_mupl->phi())));
          float calodr2 = sqrt(((CaloTower_eta[Reco_cal_size]-iTrack_mumi->eta())*(CaloTower_eta[Reco_cal_size]-iTrack_mumi->eta())) +
                               ((CaloTower_phi[Reco_cal_size]-iTrack_mumi->phi())*(CaloTower_phi[Reco_cal_size]-iTrack_mumi->phi())));
	     ...
	     it!=collTracks->end(); ++it) {
	       ...
	       fabs(dz)<3.0 && fabs(dxy)<0.3)  {

         	  if (iTrack_mupl->charge()==track->charge()) {
	               drmupl = deltaR(iTrack_mupl->eta(), iTrack_mupl->phi(), track->eta(), track->phi());
	               dptmupl = abs(1.0 - iTrack_mupl->pt()/track->pt());

                 } 
	         else if(iTrack_mumi->charge()==track->charge()){
	               drmumi=deltaR(iTrack_mumi->eta(), iTrack_mumi->phi(), track->eta(), track->phi());
	               dptmumi = abs(1.0 - iTrack_mumi->pt()/track->pt());

	         }
                if ( drmupl<0.001 &&  dptmupl<0.001 )ismupl=1;
                if ( drmumi<0.001 && dptmumi<0.001 ) ismumi=1; 
                if((ismupl ==1)||( ismumi==1)){  
                     double deta = track->eta()-CaloTower_eta[Reco_cal_size];
                     double dphi = track->phi()-CaloTower_phi[Reco_cal_size];
                    ...
                    if(dr_min<0.3) continue;
                    if (fabs(CaloTower_eta[Reco_cal_size]) < 1.479)
                      //    if ((CaloTower_id[j] == 2) || (CaloTower_id[j] == 6) || (CaloTower_id[j] == 7))
                    {
                    if (CaloTower_emE[Reco_cal_size] > leadingEmEnergy_EB)
                      {
                        leadingEmEnergy_EB = CaloTower_emE[Reco_cal_size] ;
                      }
                    }
                    if(fabs(CaloTower_eta[Reco_cal_size]) > 1.479 && fabs(CaloTower_eta[Reco_cal_size]) < 3)
          //     if ((CaloTower_id[j] == 3 || CaloTower_id[j] == 8))
                    {
                    if (CaloTower_emE[Reco_cal_size] > leadingEmEnergy_EE)
                      {
                        leadingEmEnergy_EE = CaloTower_emE[Reco_cal_size];
                      }
                    }
                    if (fabs(CaloTower_eta[Reco_cal_size])<1.305)
                      //     if ((CaloTower_id[j] == 4) || (CaloTower_id[j] ==6 ))
                    {
                    if(CaloTower_hadE[Reco_cal_size] > leadingHadEnergy_HB)
                      {
                        leadingHadEnergy_HB = CaloTower_hadE[Reco_cal_size];
                      }
                    }
                    if(fabs(CaloTower_eta[Reco_cal_size]) > 1.305 && fabs(CaloTower_eta[Reco_cal_size]) < 3)
                      // if ((CaloTower_id[j] == 5) || (CaloTower_id[j] == 7) || (CaloTower_id[j] == 8))
                    {
                    if(CaloTower_hadE[Reco_cal_size] > leadingHadEnergy_HE)
                      {
                        leadingHadEnergy_HE = CaloTower_hadE[Reco_cal_size];
                      }
                    }
                    if (CaloTower_eta[Reco_cal_size] > 3.15 && CaloTower_eta[Reco_cal_size] < 4.9)
                      //if (CaloTower_id[j] == 1 && CaloTower_eta[j]>0)
                    {
                    if(CaloTower_e[Reco_cal_size] > leadingHadEnergy_HF_Plus)
                      {
                        leadingHadEnergy_HF_Plus = CaloTower_e[Reco_cal_size] ;
                      }
                    }
                    if ( CaloTower_eta[Reco_cal_size] < -3.15 && CaloTower_eta[Reco_cal_size] > -4.9)
                      //    if (CaloTower_id[j] == 1 && CaloTower_eta[j] < 0)
                    {
                    if(CaloTower_e[Reco_cal_size] > leadingHadEnergy_HF_Minus)
                      {
                        leadingHadEnergy_HF_Minus = CaloTower_e[Reco_cal_size] ;
                      }
                    }
                    if (CaloTower_e[Reco_cal_size] > 4.0 )
                    {
                    nextracalotowersE4++;
                    }
//cout<<" leadEB "<<leadingEmEnergy_EB<<" leadEE "<<leadingEmEnergy_EE<<"  leadHB "<<leadingHadEnergy_HB<< "  leadHE "<<leadingHadEnergy_HE<<endl;

                if (
                    leadingEmEnergy_EB < 0.52 // GeV
                    &&
                    leadingEmEnergy_EE < 2.18 // GeV
                    &&
                    leadingHadEnergy_HB < 1.18 // GeV
                    &&
                    leadingHadEnergy_HE < 1.95 // GeV
                    &&
                    leadingHadEnergy_HF_Plus < 4.0 // GeV
                    &&
                    leadingHadEnergy_HF_Minus < 4.0 )       // GeV
                  {
                    // fill out exclusivity_pt_mass_eta_phi histos

                    passExclRecoCuts = true;
                 }
        Reco_cal_size++;
       }//calo loop
  if(passExclRecoCuts ==1)Reco_QQ_exclcalo[Reco_QQ_size] = 1;
  Reco_QQ_size++;
  return;
}
...
HiOniaAnalyzer::InitTree()
{
  ...
  myTree->Branch("Reco_cal_size", &Reco_cal_size,  "Reco_cal_size/I");
  myTree->Branch("CaloTower_emE",CaloTower_emE,"CaloTower_emE[Reco_cal_size]/D");
  myTree->Branch("CaloTower_hadE",CaloTower_hadE,"CaloTower_hadE[Reco_cal_size]/D");
  myTree->Branch("CaloTower_e",CaloTower_e,"CaloTower_e[Reco_cal_size]/D");
  myTree->Branch("CaloTower_et",CaloTower_et,"CaloTower_et[Reco_cal_size]/D");
  myTree->Branch("CaloTower_eta",CaloTower_eta,"CaloTower_eta[Reco_cal_size]/D");
  myTree->Branch("CaloTower_phi",CaloTower_phi,"CaloTower_phi[Reco_cal_size]/D");
  myTree->Branch("CaloTower_dr",CaloTower_dr,"CaloTower_dr[Reco_cal_size]/D");
  ...
  myTree->Branch("Reco_QQ_exclcalo", Reco_QQ_exclcalo,   "Reco_QQ_exclcalo[Reco_QQ_size]/O");
